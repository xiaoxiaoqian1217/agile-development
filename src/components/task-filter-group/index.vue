          ...
          <template v-for="(group, index) in optionGroup" :key="group.id">
            <div class="flex items-center mb-4">
              <div class="w-60px mr-14px">
                ...
                <Select class="mr-14px" v-else v-model:value="group.orAndFlag.value" :disabled="index > 1 ? true : false"
              ...
              <Select class="mr-14px" v-model:value="group.type.field" style="width: 150px"
              ...
              <div>
                ...
                " class="mr-14px w-85px" v-model:value="group.flag.value">
              ...
              <div class="w-300px">
                <div v-if="
                  ...
                  group.type.field === FilterTypeField.due_date
                ">
                  <RangePicker class="w-full" @change="(date, dateString) => dateChange(group, date, dateString)"
                    v-model:value="date[group.type.field].value" :ranges="date[group.type.field].ranges" />
                </div>
           ...
           />
                ...
                ">
                  <SelectMember class="w-full" :value="group.type.value || undefined"
                    @on-change="(value) => memeberChange(group, value)"></SelectMember>
                </div>
                <!-- 下拉框 -->
                <Select v-else class="w-full" @change="(value) => memeberChange(group, value)"
                  v-model:value="group.type.value" placeholder="待添加">
                  <SelectOption v-for="option in getOptions(group.type.field)" :value="option.id" :key="option.id">{{
                    option.name }}
                  </SelectOption>
                </Select>
              </div>
...
import { ref, inject, computed, reactive, unref, watch, onMounted, onUnmounted, nextTick } from 'vue';
